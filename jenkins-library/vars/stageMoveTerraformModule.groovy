            ...
            dir('project') {
                withCredentials([string(credentialsId: 'initialpipeline-gitcreaterepo', variable: 'GITHUB_TOKEN')]) {
                    sh """
                        ...
                        git checkout -B ${args.GitBranch}

                        git remote set-url origin "https://\${GITHUB_TOKEN}@github.com/wuttipat6509650716/project-test-infra-automation.git"

                        git status
                        git add -A

                        git diff --cached --quiet || git commit -m "chore: update terraform module ${args.TerraformModule}-${args.ServiceName}"

                        git push -u origin ${args.GitBranch}
                    """
                }
            }
        }
